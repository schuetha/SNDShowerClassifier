...
class GNN_signal_background:
    ...
    def processing(self, tree):
        ...
        for i in tqdm(range(tree.GetEntries()), desc="Processing events"):
            try:
                ...
                for hits in tree.Digi_ScifiHits:
                    ...
                    # if s_b == 1:
                    #     self.Scifi_s.GetSiPMPosition(detID, A, B) # A is left(top), B is right(bottom)
                    # elif s_b == 0:
                    #     self.Scifi_b.GetSiPMPosition(detID, A, B) # A is left(top), B is right(bottom)
                    # print(f"Event {i}: station={station}, Scifi, vert={vert}, energy={energy}")
                    # count+=1
                    # if count > 10: break
                ...
                for hits in tree.Digi_MuFilterHits:
                    ...
                    # if s_b == 1:
                    #     self.Mufi_s.GetPosition(detID, A, B)
                    # elif s_b == 0:
                    #     self.Mufi_b.GetPosition(detID, A, B)
        ...
        return features, y
        # return n_hits, flavour
        # return features, flavour
...
# python3 FCGNN/GNN_s_b.py -m train
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-g', '--geo', help='Geofiles path', default="/eos/user/s/schuetha/MC_nu/0/geofile_full.Genie-TGeant4.root", type=str)
    # parser.add_argument('-o', '--outputs', help='Output path', default="/eos/user/s/schuetha", type=str)
    args          = parser.parse_args()
    
    inputs_path_b = '/eos/experiment/sndlhc/MonteCarlo/NeutralHadrons/FTFP_BERT'
    # input_path_s = "/eos/user/s/schuetha/MC_nu"
    ...
    ranges_b      = { 'train': (1, 25),  'val': (101, 175), 'predic': (176, 400) }[args.mode]

    # self.files_name_neutrons = 'sndLHC.PG_2112-TGeant4_digCPP.root'
    # self.files_name_kaons = 'sndLHC.PG_130-TGeant4_digCPP.root'
    # self.path_end = '/Ntuples/'
    # kaons_path = f'{self.path_start}{k}/{self.kaons[row]}{self.path_end}{column}/{self.files_name_kaons}'
    # neutrons_path = f'{self.path_start}{n}/{self.neutrons[row]}{self.path_end}{column}/{self.files_name_neutrons}'
    # k = 'kaons'
    # n = 'neutrons'
    
    outputs       = '/eos/user/s/schuetha/signal_background_data_new_dataset_with_time_300_cut'
    geo           = "/eos/experiment/sndlhc/convertedData/physics/2022/geofile_sndlhc_TI18_V0_2022.root"
    # geo_b = "/eos/experiment/sndlhc/convertedData/physics/2022/geofile_sndlhc_TI18_V0_2022.root"
    # geo_s = "/eos/user/s/schuetha/MC_nu/0/geofile_full.Genie-TGeant4.root"
